...
beforeAll(async () => {
  ...
  try {
    ...
    console.warn('[tenantIsolation] DB not available — skipping integration suite:', (e as Error).message);
    return;
  }
  ...
  const orgAAdmin = await createUser({
    email: `${PFX}a-admin@example.com`, password: 'unique-admin-pw-1',
    name: 'Iso A Admin', role: 'admin', org_id: orgA!.id,
  });
  const orgAOperator = await createUser({
    email: `${PFX}a-op@example.com`, password: 'unique-operator-pw-1',
    name: 'Iso A Op', role: 'operator', org_id: orgA!.id,
  });
  const orgAViewer = await createUser({
    email: `${PFX}a-view@example.com`, password: 'unique-viewer-pw-1',
    name: 'Iso A Viewer', role: 'viewer', org_id: orgA!.id,
  });
  const orgBAdmin = await createUser({
    email: `${PFX}b-admin@example.com`, password: 'unique-admin-pw-2',
    name: 'Iso B Admin', role: 'admin', org_id: orgB!.id,
  });
  const orgBViewer = await createUser({
    email: `${PFX}b-view@example.com`, password: 'unique-viewer-pw-2',
    name: 'Iso B Viewer', role: 'viewer', org_id: orgB!.id,
  });
  const superAdmin = await createUser({
    email: `${PFX}super@example.com`, password: 'unique-super-pw-1',
    name: 'Iso Super', role: 'super_admin', org_id: orgA!.id,
  });
  ...
  const locA1 = await createLocation({
    name: `${PFX}site-shared`, site_type: 'mine', geom: POLY, centroid: PT, org_id: orgA!.id,
  });
  const locA2 = await createLocation({
    name: `${PFX}site-a-only`, site_type: 'mine', geom: POLY, centroid: PT, org_id: orgA!.id,
  });
  const locB1 = await createLocation({
    name: `${PFX}site-shared`, site_type: 'mine', geom: POLY, centroid: PT, org_id: orgB!.id,
  });
  ...
  const alertA = await addAlert({
    location_id: locA1.id, state_id: stateA!.id,
    alert_type: 'email', recipient: `${PFX}rec-a-alert@example.com`,
    sent_at: new Date().toISOString(), delivered_at: null,
    error: null, escalated: false, acknowledged_at: null, acknowledged_by: null,
    twilio_sid: null,
  });
  const alertB = await addAlert({
    location_id: locB1.id, state_id: stateB!.id,
    alert_type: 'email', recipient: `${PFX}rec-b-alert@example.com`,
    sent_at: new Date().toISOString(), delivered_at: null,
    error: null, escalated: false, acknowledged_at: null, acknowledged_by: null,
    twilio_sid: null,
  });
...
afterAll(async () => {
  ...
  await query(`DELETE FROM organisations WHERE id = ANY($1::uuid[])`, [[state.orgAId, state.orgBId]]);
...
describe.skipIf(!dbAvailable)('tenant isolation — canAccessLocation predicate (route-level gate)', () => {
...
describe.skipIf(!dbAvailable)('tenant isolation — alert acknowledgement (POST /api/ack/:alertId pattern)', () => {
  ...
  it('an org A operator cannot ack an org B alert (rowCount = 0)', async () => {
    ...
    const after = await getOne<{ acknowledged_at: string | null }>(
      'SELECT acknowledged_at FROM alerts WHERE id = $1', [state!.alertBId]
    );
...
describe.skipIf(!dbAvailable)('tenant isolation — recipients are reachable only via the owning location', () => {
  it('addLocationRecipient + getLocationRecipients stay org-scoped', async () => {
    await addLocationRecipient({
      location_id: state!.locA1Id, email: `${PFX}rec-a@example.com`, phone: null,
      active: true, notify_email: true, notify_sms: false, notify_whatsapp: false,
    });
    await addLocationRecipient({
      location_id: state!.locB1Id, email: `${PFX}rec-b@example.com`, phone: null,
      active: true, notify_email: true, notify_sms: false, notify_whatsapp: false,
    });
