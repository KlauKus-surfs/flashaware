        ...
        <TableCell sx={{ fontSize: 11, color: 'text.secondary' }}>
          <Tooltip title={row.target_id || ''}>
            <Box
              onClick={() => {
                if (row.target_id) {
                  onTargetClick(row.target_id);
                }
              }}
              sx={{
                cursor: row.target_id ? 'pointer' : 'default',
                '&:hover': row.target_id ? { textDecoration: 'underline' } : {},
              }}
              component="span"
            >
              {row.target_id
                ? row.target_id.slice(0, 12) + (row.target_id.length > 12 ? '…' : '')
                : '—'}
            </Box>
          </Tooltip>
        </TableCell>
  ...
  return (
    <Box>
      ...
      <Alert severity="info" sx={{ mb: 2, fontSize: 12 }}>
        <strong>What's audited:</strong> user logins, location / recipient / org / user / settings /
        invite mutations, alert acknowledgements and test sends. <strong>What's not:</strong>{' '}
        auto-fired alert dispatch (use Alert History instead) and read-only browsing. Mutations that
        happened before this organisation enabled audit logging won't appear here.
      </Alert>
      ...
      <Card sx={{ mb: 2 }}>
        <CardContent sx={{ '&:last-child': { pb: 2 } }}>
          ...
          >
            ...
            />
            <Box sx={{ flex: 1 }} />
            <IconButton aria-label="Refresh" onClick={fetchRows} disabled={loading}>
              <RefreshIcon />
            </IconButton>
          </Stack>
        </CardContent>
