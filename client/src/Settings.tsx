...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      ...
      getSettings(scopedOrgId ?? undefined)
        .then((res) => {
    ...
    <Box>
            ...
            <Grid container spacing={3}>
              <Grid item xs={6} sm={3}>
                <Paper sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.03)', textAlign: 'center' }}>
                  ...
                  <Chip
                    label={health.status === 'ok' ? 'Online' : 'Error'}
                    size="small"
              ...
              <Grid item xs={6} sm={3}>
                ...
                >
                  ...
                  {(() => {
                    const tier =
                      (health.feedTier as string | undefined) ??
                      (health.feedHealthy ? 'healthy' : 'stale');
                    const cfg: Record<
                      string,
                    ...
                    return (
                      <Chip label={c.label} size="small" color={c.color} sx={{ fontWeight: 600 }} />
                    );
                  })()}
                </Paper>
              </Grid>
              ...
              <Grid item xs={6} sm={3}>
                <Paper sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.03)', textAlign: 'center' }}>
                  ...
                  <Typography
                    ...
                    sx={{
                      fontSize: 20,
                      color: (health.dataAgeMinutes ?? 99) < 25 ? 'success.main' : 'error.main',
                    }}
                  >
          ...
          <AccordionDetails>
            {isSuperAdmin &&
              ...
              (scopedOrgId ? (
                <Alert severity="info" sx={{ mb: 2 }}>
                  Editing settings for <strong>{scopedOrgName}</strong>. Use the org picker in the
                  top bar to switch tenants. Empty values fall back to platform defaults.
                </Alert>
              ) : (
                <Alert severity="warning" sx={{ mb: 2 }}>
                  <strong>
                    No tenant selected — saving will write to FlashAware (your platform tenant).
                  </strong>{' '}
                  Pick a customer in the top-bar org picker to target their settings instead. For
                  platform-wide defaults, super admins should use the dedicated platform-settings
                  endpoint.
                </Alert>
              ))}
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                  ...
                  <FormControlLabel
                    control={
                      <Switch
                        ...
                        onChange={(e) =>
                          setNotifications({ ...notifications, whatsappEnabled: e.target.checked })
                        }
                      />
                    }
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <FormControlLabel
                      control={
                        <Switch
                          checked={notifications.escalationEnabled}
                          onChange={(e) =>
                            setNotifications({
                              ...notifications,
                    ...
                    <InfoTip
                      inline
                      title={helpTitle('auto_escalation')}
                      body={helpBody('auto_escalation')}
                    />
                  </Box>
                </Box>
                <Typography
                ...
                >
                  These flags gate dispatch at the org level. Per-recipient channel toggles still
                  apply on top — both must be on for a message to send. SMS / WhatsApp also require
                  an OTP-verified phone.
                </Typography>
              </Grid>
              <Grid item xs={6} sm={4}>
                <TextField
                  ...
                  onChange={(e) =>
                    setNotifications({
                      ...notifications,
                      escalationDelayMin: Math.max(1, +e.target.value),
                    })
                  }
                  inputProps={{ min: 1 }}
                  InputProps={{
                    endAdornment: (
                      <InfoTip
                        inline
                        title={helpTitle('escalation_delay')}
                        body={helpBody('escalation_delay')}
                      />
                    ),
                  }}
              ...
              <Grid item xs={6} sm={4}>
                <TextField
                  fullWidth
                  label="Alert From Address"
                  size="small"
                  value={notifications.alertFromAddress}
                  onChange={(e) =>
                    setNotifications({ ...notifications, alertFromAddress: e.target.value })
                  }
            ...
            {health?.mode === 'in-memory-mock' && (
              <Alert severity="info" sx={{ mt: 2 }}>
                Running in mock mode — emails are logged to console only. Configure SMTP credentials
                in <code>.env</code> for production.
              </Alert>
            )}
            ...
            <Box sx={{ mt: 3, p: 2, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.03)' }}>
              ...
              >
                Confirms SMTP credentials are working. The recipient will see a "Test Alert" message
                with no real lightning context.
              </Typography>
      ...
      {isAdmin && (
        <Accordion sx={{ mb: 2, bgcolor: 'background.paper', '&:before': { display: 'none' } }}>
          ...
          <AccordionDetails>
            <Alert severity="info" sx={{ mb: 2 }}>
              These are the platform-wide retention defaults applied by the cleanup job. A future
              release will let admins edit these from this page.
            </Alert>
            <Grid container spacing={2}>
              <Grid item xs={6} sm={3}>
                <TextField
                  fullWidth
                  label="Flash Events (days)"
                  type="number"
                  size="small"
            ...
            {health?.mode === 'in-memory-mock' && (
              <Alert severity="warning" sx={{ mt: 2 }}>
                In-memory mock mode — all data is lost on server restart. Data retention policies
                only apply in production with PostgreSQL.
              </Alert>
            )}
          </AccordionDetails>
        </Accordion>
