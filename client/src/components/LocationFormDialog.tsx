...
export function LocationFormDialog({
  ...
  pendingEmails,
  testingRecipientId,
  onAddRecipientPersisted,
  onAddRecipientPending,
  onRemovePendingEmail,
  onUpdateRecipient,
  onDeleteRecipient,
  onSendTestRecipient,
  onStartVerifyRecipient,
}: Props) {
  return (
    <Dialog
      open={open}
    ...
    >
      <DialogTitle>
        ...
        {!editing && isSuperAdmin && !scopedOrgName && (
          <Typography
            variant="caption"
            sx={{ display: 'block', color: 'warning.main', fontWeight: 400 }}
          >
            No org selected — will be created in <strong>FlashAware</strong>. Use the picker in the
            top bar to target a customer org.
          </Typography>
        )}
      </DialogTitle>
      <DialogContent>
        <Grid container spacing={2} sx={{ mt: 0.5 }}>
          ...
          <Grid item xs={6} sm={3}>
            <TextField
              fullWidth
              label="All Clear Wait (min)"
              type="number"
              size="small"
              value={form.allclear_wait_min}
              helperText={
                fieldErrors.allclear_wait_min ??
                'Quiet minutes required before returning to ALL CLEAR'
              }
              inputProps={{ min: 1 }}
          ...
          {!form.alert_on_change_only && (
            <Grid item xs={6} sm={3}>
              <TextField
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, persistence_alert_min: Math.max(1, +e.target.value) }))
                }
              />
            </Grid>
          )}
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, alert_on_change_only: e.target.checked }))
                  }
                  color="warning"
                />
              }
              label={
                <Box>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <InfoTip
                      inline
                      title={helpTitle('state_change_only')}
                      body={helpBody('state_change_only')}
                    />
                  </Box>
                  <Typography variant="caption" color="text.secondary">
                    Only notify on transitions (e.g. ALL CLEAR → STOP → ALL CLEAR). No repeat alerts
                    while storm persists. Ideal for wind farms.
                  </Typography>
                </Box>
              }
            />
            ...
            <FormControlLabel
              ...
              label={
                <Box>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <Typography variant="body2" sx={{ fontWeight: 500 }}>
                      Mark as demo / test data
                    </Typography>
                    <InfoTip
                      inline
                      title={helpTitle('demo_location')}
                      body={helpBody('demo_location')}
                    />
                  </Box>
                  <Typography variant="caption" color="text.secondary">
                    Hides this location from the main dashboard until "Show demo" is toggled on.
                    Risk engine still evaluates it so test alerts work.
                  </Typography>
                </Box>
              }
            />
          ...
          {form.stop_radius_km > 0 &&
            form.stop_radius_km < STOP_RADIUS_WARNING_THRESHOLD_KM &&
            !form.is_demo && (
              <Grid item xs={12}>
                <Alert severity="warning" sx={{ fontSize: 12, py: 0.5 }}>
                  <strong>
                    {form.stop_radius_km} km STOP radius is below the EUMETSAT MTG-LI footprint (4.5
                    km at the sub-satellite point, ≤10 km at 45° latitude per the official MTG spec{' '}
                    <Box
                      component="a"
                  ...
                  Over Southern Africa the per-flash footprint is typically 5–8 km, so a real strike
                  on the site centroid will often plot outside this radius and the engine will
                  silently miss it. Consider ≥ 5 km unless this is a calibration / ground-truth
                  site.
                </Alert>
              </Grid>
            )}
